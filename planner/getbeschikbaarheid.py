...
def write_markup(filename, rooster):
    ...
    for week in sorted(weeks):
        ...
        file.write('|')
        file.write("{:<9s}".format(rooster[week]['leider']))
        file.write('|')
        file.write("{:<8s}".format(rooster[week]['team']))
        file.write('|')
        file.write("{:<8s}".format(rooster[week]['schuiver']))
        file.write('|')
        file.write("{:<8s}".format(rooster[week]['beamer']))
        file.write('|')
        file.write("{:<8s}".format(rooster[week]['leider_blauw']))
        file.write(' & ')
        file.write("{:<8s}".format(rooster[week]['helper_blauw']))
        file.write('|')
        file.write("{:<8s}".format(rooster[week]['leider_wit']))
        file.write(' & ')
        file.write("{:<8s}".format(rooster[week]['helper_wit']))
        file.write('|')
        file.write("{:<11s}".format(rooster[week]['leider_rood']))
        file.write('|')
        file.write("{:<9s}".format(rooster[week]['koffie']))
        file.write('|')
        file.write("{:<8s}".format(rooster[week]['koster']))
        file.write(' & ')
        file.write("{:<16s}".format(rooster[week]['hulpkoster']))
        file.write('|')
        file.write("{:<16s}".format(rooster[week]['gastvrouw']))
        file.write('|')
